...
(defun gen-render-code (render-exp)
  (if (consp render-exp)
      (case (car render-exp)
        ...
        (hi `(with-attr (reverse)
               ,@(mapcar #'gen-render-code (cdr render-exp))))
        (b `(with-attr (bold)
              ,@(mapcar #'gen-render-code (cdr render-exp))))
        (u `(with-attr (underline)
              ,@(mapcar #'gen-render-code (cdr render-exp))))
        ...
        (c `(with-color ,(let ((c (cadr render-exp)))
                              (if (get-color c)
                                  c 'black))
              ,@(mapcar #'gen-render-code (cddr render-exp))))
        (lisp (let ((name (gensym))
                    (lisp (cdr render-exp)))
                `(with-cs ((,name ,lisp))
                   (wprintw *stdscr* ,name))))
...
(if (= 1 (length sb-ext:*posix-argv*))
    ...
    (if (string= (nth 1 sb-ext:*posix-argv*)
        ...
        (sb-ext:save-lisp-and-die
         "slider" :executable t :compression 3
         :toplevel #'slider:main)
        (slider:main)))
